...
with open('trainHistoryDict_loc.txt', 'rb') as file_pi:
...
# 加载模型用做预测
json_file = open(r"./model_loc.json", "r")
...
Rev1_test_data = np.load("Rev1_test_data_loc.npy")
Rev2_test_data = np.load("Rev2_test_data_loc.npy")
test_label = np.load("test_label_loc.npy")
...
# 自定义坐标轴标签
fontdict = {'fontsize': 8, 'fontweight': 'bold', 'fontfamily': 'Times New Roman'}
plt.xlabel('Predicted number', fontdict=fontdict)  # 更改预测标签的显示
plt.ylabel('True number', fontdict=fontdict)       # 更改真实标签的显示

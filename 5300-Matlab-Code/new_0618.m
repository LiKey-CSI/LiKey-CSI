...
csi_trace1 = read_bf_file('./data0618/Rev_W/loc4_p111_2.dat'); % 数据文件路径
csi_trace2 = read_bf_file('./data0618/Rev_Z/loc4_p111_2.dat'); % 数据文件路径
...
%% 读取Excel的分割结果
[ndata, text, alldata] = xlsread('按键分割.xlsx','Sheet1','B10:D10');
point_data = [];
for i = 1:length(text)
...
for i = 1:3
    ...
    eval(['Rev1_number1_',num2str(i+start_pos),'=allsub_feature_REV1_r1r2;']); % 随运行文件需要修改
    eval(['Rev2_number1_',num2str(i+start_pos),'=allsub_feature_REV2_r1r2;']); % 随运行文件需要修改
    figure;
    colormap(gray);
    subplot(2,1,1);
...
% 30个子载波合为一个
for j = 1:3
    eval(['number_',num2str(j),'=[mode(Rev1_number1_',num2str(j+start_pos),',1),mode(Rev2_number1_',num2str(j+start_pos),',1)];']);
end
...
for i = 1:3
    ...
    eval(['Rev1_abs_number1_',num2str(i+start_pos),'=rev1_waveform_abs;']);
    eval(['Rev2_abs_number1_',num2str(i+start_pos),'=rev2_waveform_abs;']);
    eval(['Rev1_phase_number1_',num2str(i+start_pos),'=rev1_waveform_phase;']);
    eval(['Rev2_phase_number1_',num2str(i+start_pos),'=rev2_waveform_phase;']);
...
for i = 1:3
    save(['./0618按键特征-new/single_number/Rev1/按键1/Rev1_number1_',num2str(i+start_pos),'.mat'],['Rev1_number1_',num2str(i+start_pos)]);
    save(['./0618按键特征-new/single_number/Rev2/按键1/Rev2_number1_',num2str(i+start_pos),'.mat'],['Rev2_number1_',num2str(i+start_pos)]);
    
    save(['./0618按键特征-new/波形特征/振幅/Rev1/按键1/Rev1_abs_number1_',num2str(i+start_pos),'.mat'],['Rev1_abs_number1_',num2str(i+start_pos)]);
    save(['./0618按键特征-new/波形特征/振幅/Rev2/按键1/Rev2_abs_number1_',num2str(i+start_pos),'.mat'],['Rev2_abs_number1_',num2str(i+start_pos)]);

    save(['./0618按键特征-new/波形特征/相位/Rev1/按键1/Rev1_phase_number1_',num2str(i+start_pos),'.mat'],['Rev1_phase_number1_',num2str(i+start_pos)]);
    save(['./0618按键特征-new/波形特征/相位/Rev2/按键1/Rev2_phase_number1_',num2str(i+start_pos),'.mat'],['Rev2_phase_number1_',num2str(i+start_pos)]);
end